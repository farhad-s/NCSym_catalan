...
If the sequences $\vec{a}$ are non-negative then we can say more about $\vec{a}$, $\vec{h}$ and $\vec{p}$ because
they will have combinatorial interpretations as subsets of words in an alphabet.  To this end, we require a few
definitions.

Let $X = \bigcup_{i \geq 1} X^{(i)}$ be an alphabet of letters for which $|X^{(i)}| < \infty$
where $X^{(i)}$ is the finite set of letters of degree $i$.
We denote the degree of $x \in X^{(i)}$ by $\mathsf{deg}(x) := i$.
A word in $X$ is a concatenation of letters
$w = w_1 w_2 \ldots w_m$ where $w_r \in X$ and the degree of $w$ is denoted $\mathsf{deg}(w) = \sum_{j = 1}^m \mathsf{deg}(w_j)$.

Place a total order on the alphabet and the dictionary order on the words imposes a total order which is
normally referred to as lexicographic order.
For $0 < a < m$, a cyclic shift of $w$ is denoted $cyc^a(w) = w_{a+1} w_{a+1} \cdots w_n w_1 w_2 \cdots w_a$.
We say that $w$ is a Lyndon word if $w$ is strictly smaller than $cyc^a(w)$ in lexicographic order for all $0 < a < m$.
For instance, if $m=1$ and $w = w_1$, then $w$ is a Lyndon word since there are no cyclic shifts.
If $x < y$, then $xyxyy$ is a Lyndon word, but $w = xyxy$ is not because $cyc^2(w) = w$.

\begin{proposition}
Assume that the sequences $\vec{a}$, $\vec{h}$ and $\vec{p}$ are related by Equation \eqref{eq:gf_relation}
and that $\vec{a} \geq \vec{0}$, then let $X = \bigcup_{i \geq 1} X^{(i)}$ be a graded alphabet where
$|X^{(i)}| = a_i$ for all $i \geq 1$ and the degree of a letter $x \in X^{(i)}$ is assumed to be of degree $i$.
Then,
\begin{itemize}
\item for $n\geq 1$, $h_n$ is equal to the number of words in the alphabet $X$ of degree $n$
\item for $n\geq 1$, $p_n$ is equal to the number of Lyndon words in the alphabet $X$ of degree $n$~.
\end{itemize}
\end{proposition}

\begin{proof}
For a composition $\beta \vdash n$, $a_\beta$ is equal to the number of words
of length $m$ with $w = w_1 w_2 \cdots w_m$, with $\beta_i = \mathsf{deg}(w_i)$ and $w_i \in X^{(\beta_i)}$.
That is, $\beta$ is the sequences of the degrees of the letters of $w$.
Since by Proposition \ref{prop:sequences}, we have that $h_n = \sum_\beta a_\beta$
and every word has some sequence of degrees of the letters, it follows that
$h_n$ is equal to the total number of words of degree $n$.
...
it suffices to show that \eqref{eq:gf_relation} holds. The number of monomials of degree m is  $\displaystyle 
   ...
   $ 1 + \sum_{m \geq 1} h_m t^m = 
   \sum_{k\geq 0} (\sum
    \limits_{\substack{\beta \vDash m \\ l(\beta)=k } }a_\beta t^m)=
   \sum_{k \geq 0} \left(\sum_{m \geq 1} a_m t^m\right)^k$. Which in turn is equal to 
$\frac{1}{1 - \sum_{m \geq 1} a_m t^m}$
 using the expansion $\frac{1}{1-x}= \sum_{k\geq 0} x^k $.

On the other hand, each monomial of degree $m$ is a product of primitive generators. Therefore the number of polynomials of degree $m$ is the number of solutions to the equation $\sum\limits_{\substack {d}}\sum\limits_{\substack{ d_i\leq p_d}}  c_{d_i}d=m$. The generating function associated to this equation is
...
$1 + \sum_{k \geq 1} h_k t^k=\prod_{d \geq 1} \frac{1}{(1-t^d)^{p_d}}$.
\end{proof}
...
Let $H$ be a free graded connected cocommutative Hopf algebra and let $\vec{p} = (p_{1}, p_{2}, \ldots)$ be the sequence defined by $p_{n} = \dim(\mathcal{P}(H)_{n})$.  Then there is a bijection
\[
\begin{array}{rcl}
\left\{\begin{array}{c}
...
The proof of the theorem follows several examples and intermediate results.
...
\[
\dim(\mathfrak{L}(\vec{a})_{n}) - \dim([\mathfrak{L}(\vec{a}), \mathfrak{L}(\vec{a})]_{n}) = b_{n}.
\]
\end{lemma}
\begin{proof}
\lucas{TODO}
